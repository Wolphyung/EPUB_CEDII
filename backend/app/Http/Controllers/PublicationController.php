   ...
   public function store(Request $request)
{
    $validated = $request->validate([
        ...
        'image' => 'nullable|image|max:5120', // augmente à 5 Mo
    ]);
    ...
    // 🔹 Gestion de l'image avec URL publique
    if ($request->hasFile('image')) {
        $path = $request->file('image')->store('publications', 'public');
        // Générer l’URL complète (ex: http://localhost:8000/storage/publications/image.jpg)
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'statut' => 'nullable|in:Brouillon,En attente,Validé',
            'id_utilisateur' => 'nullable|exists:utilisateurs,id_utilisateur',
            'image' => 'nullable|image|max:5120', // 5120 = 5 Mo,
        ]);
